...
<script>
    ...
    const initData = async (props = null, tanggal = "") => {
        ...
        const gets = await axios.get("<?= base_url("RekapAdmin/getDataJenisBySearch/" . $this->uri->segment(3) . "/") ?>" + tanggal + param).catch(() => {
            console.log("error");
        });
    ...
    $("#jenis_sampah").change(function() {
        ...
        if ($(this).val() == "") {
            ...
            async function getByKode(kode) {
                const getter = await axios.get("<?= base_url("Transaksi/nama_barang_sampah/" . $this->uri->segment(3) . "/") ?>" + kode).catch(() => {
                    console.log("err");
                });
    ...
    $("[name='nama_barang']").change(function() {
        ...
        if ($(this).val() == "") {
            ...
            async function getByKode(kode) {
                const getter = await axios.get("<?= base_url("RekapAdmin/barangbyid/" . $this->uri->segment(3) . "/") ?>" + kode).catch(() => {
                    console.log("err");
                });
                if (getter?.status ?? 400 == 200) {
                    const data = getter.data;
...
<script>
    ...
    const initDataChart = async (tanggal = "") => {
        const gets = await axios.get("<?= base_url("RekapAdmin/chartDataRekap/" . $this->uri->segment(3) . "/") ?>" + tanggal).catch(() => {
            console.log("error");
        });
        if (gets?.status ?? 400 == 200) {
...
<script>
    ...
    const initRekapThn = async (tahun = null) => {
        ...
        const gets = await axios.get("<?= base_url("RekapAdmin/getDataRekaptahunan/" . $this->uri->segment(3) . "/") ?>" + thn).catch(() => {
            console.log("error");
        });
        if (gets?.status ?? 400 == 200) {
    ...
    $("#print-berat-tahunan").click(function() {
        ...
        window.open('<?= base_url("Printing/RekapBeratTahunanPrint/" . $this->uri->segment(3) . "/") ?>' + tahun, '_blank')
    })
</script>